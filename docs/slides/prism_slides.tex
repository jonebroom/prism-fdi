...
%% ─── 网站介绍：PRISM平台一览 ────────────────────────────────
\begin{frame}{我们建了什么：PRISM平台}
  \begin{columns}[T]
    \begin{column}{0.50\linewidth}
      \textbf{\color{prismclay}PRISM}（Policy Review and Investment Screening Mechanisms）\\
      是一个交互式外商直接投资审查机制数据平台\\[0.8em]
      \begin{tcolorbox}[colback=bluebg, colframe=prismblue, arc=3pt,
        left=6pt, right=6pt, top=4pt, bottom=4pt]
        \begin{tabular}{ll}
          \textbf{覆盖国家} & 38个（OECD·EU/EEA·五眼）\\
          \textbf{时间跨度} & 2007—2023年（17年）\\
          \textbf{时序记录} & 650条国家-年数据\\
          \textbf{立法事件} & 143项\\
          \textbf{政策文本} & 141份原文（PDF+HTML）\\
          \textbf{行业指标} & 36个细分 + 8个汇总类\\
        \end{tabular}
      \end{tcolorbox}
      \vspace{0.6em}
      \begin{exampleblock}{访问地址（无需登录）}
        \centering\small\ttfamily jonebroom.github.io/prism-fdi/
      \end{exampleblock}
    \end{column}
    \begin{column}{0.47\linewidth}
      \imgbox{7cm}{平台Overview Tab截图}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{六个分析模块}
  ...
  \begin{tabularx}{\linewidth}{clX}
    \toprule
    \textbf{Tab} & \textbf{名称} & \textbf{功能} \\
    \midrule
    01 & Overview  & 世界地图着色·全球趋势折线图·年度快照统计 \\
    02 & Country   & 单国深度档案：雷达图·KPI卡·法规气泡时间线·13项指标格 \\
    03 & Comparison & 多国平行坐标图：拖动坐标轴筛选国家子集 \\
    04 & Evolution & 覆盖类型演变·严格程度分布·通知×预审批矩阵·法规替代网络 \\
    05 & Changes   & 立法活动柱状图·逐年事件列表·点击同步全局年份滑块 \\
    06 & Sectors   & 36行业×38国热力图·行业排名赛马动图 \\
    \bottomrule
  \end{tabularx}
  \end{table}
  \vspace{0.3em}
  \begin{columns}[T]
    \begin{column}{0.33\linewidth}
      \begin{block}{\small 全局控件（左侧栏）}
        \footnotesize
        年份滑块（2007—2023）\\
        ▶ 播放时间轴动画\\
        OECD / EU/EEA / Five Eyes 筛选\\
        国家列表（点击跳转）
      \end{block}
    \end{column}
    \begin{column}{0.33\linewidth}
      \begin{alertblock}{\small Deal Screener}
        \footnotesize
        ⚡ Check Deal\\
        输入目标国、行业、拟收购比例\\
        → 自动判断是否需要申报
    ...
    \end{column}
    \begin{column}{0.33\linewidth}
      \begin{exampleblock}{\small Ask AI}
        \footnotesize
        自然语言提问PRISM数据\\
        支持Claude·GPT·DeepSeek\\
        等多个模型后端
      \end{exampleblock}
    \end{column}
  \end{columns}
...
\begin{frame}{起点：我有一份政策数据集}
  \begin{columns}[T]
    \begin{column}{0.52\linewidth}
      ...
      \begin{itemize}
        \item \textbf{38个国家} × \textbf{17年}（2007—2023）逐年编码
        \item \textbf{13项程序性指标}：是否强制预审批、门槛、时限……
