...
(function(){
  ...
  function getTYPES(){ return [
    {key:'new_law',     label:t('chg_type_law'),   cls:'law',   color:'#4f6f9e'},
    {key:'new_amendment',label:t('chg_type_amend'),cls:'amend', color:'#7d97b8'},
    {key:'new_eo',      label:t('chg_type_eo'),    cls:'eo',    color:'#c79a3e'},
    {key:'new_reg_impl',label:t('chg_type_reg'),   cls:'reg',   color:'#b6bcc6'},
  ]; }
  ...
  function mount(el){
      ...
      <div class="stack">
        <div class="panel">
          <div class="panel-head">
            <div class="titles"><h2>${t('chg_bar_title')}</h2><p>${t('chg_bar_sub')}</p></div>
        ...
        <div class="panel">
          <div class="panel-head">
            <div class="titles"><h2 id="chgYrTitle">${t('chg_list_title')}</h2><p id="chgYrSub">${t('chg_list_sub')}</p></div>
    ...
    root.querySelector('#chgLegend').innerHTML=TYPES.map(tp=>`<div class="lg"><span class="sw" style="background:${tp.color}"></span>${tp.label}</div>`).join('');
  ...
  function drawChart(){
    ...
    const series=TYPES.map(tp=>({
      name:tp.label,type:'bar',stack:'a',data:years.map(y=>gy[y]?gy[y][tp.key]:0),
      itemStyle:{color:tp.color},barWidth:'58%',
    ...
    chart.setOption({
      tooltip:Object.assign(P.EC.tip(),{trigger:'axis',axisPointer:{type:'shadow'},formatter:ps=>{
        ...
        h+=P.tipRow('<b>'+t('tip_total')+'</b>','<b>'+tot+'</b>'); return h;
      }}),
      ...
      yAxis:P.EC.axis({type:'value',name:t('chg_new_mech_y')}),
      series,
      graphic:annot(years)
    },true);
    highlight();
  ...
  function drawList(){
    ...
    root.querySelector('#chgYrTitle').textContent=selYear+' '+t('chg_yr_events');
    const gy=P.DATA.yearly.global_yearly[selYear];
    root.querySelector('#chgYrSub').textContent=gy?`${gy.total_new} ${t('chg_new_global')} · ${evs.length} ${t('chg_events_sfx')}`:`${evs.length} ${t('chg_events_sfx')}`;
    root.querySelector('#chgCount').textContent=evs.length+' '+t('chg_events_sfx');
    if(!evs.length){ root.querySelector('#chgList').innerHTML=`<div class="empty"><span class="big">${t('chg_no_events')}</span></div>`; return; }
    root.querySelector('#chgList').innerHTML=`<div class="evlist">`+evs.map((e,i)=>{
      const tags=[];
      if(e.new_law)tags.push(`<span class="chip law">${t('chg_chip_law')}</span>`);
      if(e.new_amendment)tags.push(`<span class="chip amend">${t('chg_chip_amend')}</span>`);
      if(e.new_eo)tags.push(`<span class="chip eo">${t('chg_chip_eo')}</span>`);
      if(!e.new_law&&!e.new_amendment&&!e.new_eo)tags.push(`<span class="chip reg">${t('chg_chip_impl')}</span>`);
      return `<div class="evrow" data-i="${i}" style="cursor:pointer;">
        <div class="ev-yr">${e.year}</div>
        <div class="ev-body">
          <div class="ev-name">${isWeakName(e.name)?`<span style="color:var(--ink-soft);font-style:italic">${t('chg_ref')}${e.name||t('sr_untitled')}</span>`:e.name}${e.superceded?`<span class="chip super">${t('chg_chip_super')}</span>`:''}</div>
          ${e.explanation?`<div class="ev-exp">${e.explanation}</div>`:''}
          <div class="ev-ctry">${UI.cnShort(e.country)}</div>
        </div>
  ...
  function openChgDetail(e){
    ...
    const fileLink=doc?(doc.ext==='.pdf'
      ?`<a href="policy_files/policy_files/${encodeURIComponent(doc.filename)}" download="${esc(doc.filename)}" style="display:inline-flex;align-items:center;gap:4px;padding:6px 12px;border-radius:20px;border:1px solid var(--line-strong);font-size:12px;color:var(--ink-soft);text-decoration:none">${t('ct_download_pdf')}</a>`
    ...
    const detail=`
      <div class="dr-section">
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">
          ...
          ${tags.join('')}
          ${e.superceded?`<span class="chip super">${t('chg_chip_super')}</span>`:''}
        </div>
        ...
        ${rg?`<div style="font-size:11px;color:var(--ink-faint);margin-bottom:6px;font-style:italic">${t('ct_matched_reg')}"${esc(rg.regulation)}"</div>`:''}
        <div class="inforows">
          ${rg?`<div class="inforow"><span class="ir-k">${t('chg_detail_lead')}</span><span class="ir-v txt">${esc(rg.lead_authority||'—')}</span></div>`:''}
          ${rg?`<div class="inforow"><span class="ir-k">${t('chg_detail_thresh')}</span><span class="ir-v">${rg.threshold!=null?(rg.threshold*100).toFixed(0)+'%':'—'}</span></div>`:''}
          ${rg?`<div class="inforow"><span class="ir-k">${t('chg_detail_tf')}</span><span class="ir-v">${rg.time_frame_days!=null?rg.time_frame_days+' '+t('ct_days'):'—'}</span></div>`:''}
          ${rg?`<div class="inforow"><span class="ir-k">${t('chg_detail_strict')}</span><span class="ir-v">${P.strictness(rg)} / 13</span></div>`:''}
          ${rg?`<div class="inforow"><span class="ir-k">${t('chg_detail_sectors')}</span><span class="ir-v">${P.sectorCount(rg)} / ${P.DATA.sectorNames.length}</span></div>`:''}
          ${rg&&rg.ns_test?`<div class="inforow"><span class="ir-k">${t('chg_detail_ns')}</span><span class="ir-v">✓</span></div>`:''}
          ${!rg?`<div class="inforow"><span class="ir-k" style="color:var(--ink-faint);font-style:italic">${t('ct_no_match')}</span></div>`:''}
          ${e.superceded?`<div class="inforow"><span class="ir-k">${t('chg_detail_super')}</span><span class="ir-v txt">${esc(e.superceded)}</span></div>`:''}
        </div>
        <div style="display:flex;gap:8px;margin-top:14px;">
          <button class="ai-chip" style="flex:1;text-align:center" onclick="window.PRISM_UI.closeDrawer();window.PRISM_UI.selectCountry('${e.country.replace(/'/g,"\\'")}');window.PRISM_UI.switchTab('country')">${UI.cnShort(e.country)} ${t('chg_view_country')}</button>
        </div>
      </div>
      ${rawText?`<div class="dr-section"><h3>${t('ct_policy_text')}${truncated?' '+t('ct_policy_excerpt'):''} · ${t('ct_doc_chars').replace('{n}',doc.char_count.toLocaleString())}</h3>
        ...
        <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">${fileLink}
          <button class="ai-chip" style="flex:1;text-align:center" onclick="window.PRISM_AI.askAbout('Summarize the core screening mechanism of this regulation: ${(e.name||'').replace(/'/g,'').replace(/\\/g,'').replace(/"/g,'')}')">${t('ct_summarize_ai')}</button>
        </div></div>`
      :''}`;
    // Display name: if bare ref/date, label it clearly
    const displayName = weakName
      ? (e.name ? t('chg_ref')+e.name : t('chg_event'))
      : (e.name || t('chg_event'));
    UI.openDrawer(displayName, `${UI.cnShort(e.country)} · ${e.year}`, detail);
  }
